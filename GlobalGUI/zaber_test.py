...
with Connection.open_serial_port("COM8") as connection:
    ...
    connection.generic_command(f"move abs {position_absolute}", 1, 0, True, 500) # X zaber
    
    actual_status = connection.generic_command("get motion.busy", 1, 0, True, 500)
    while actual_status.status == 'BUSY':
      actual_status = connection.generic_command("get motion.busy", 1, 0, True, 500)
      
    connection.generic_command(f"move abs {position_absolute}", 2, 0, True, 500) # Y Zaber
